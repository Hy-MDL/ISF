...
def main(args):
    ...
    data_path = "/home/hyeonmin/ISF/timegan-pytorch-main/data/sliding_windows_indexed.csv"
    X, T, _, args.max_seq_len, args.padding_value, uniq_id, dropped_indices = data_preprocess(
    ...
    train_data, test_data, train_time, test_time, train_idx, test_idx = train_test_split(
        X, T, uniq_id, test_size=args.train_rate, random_state=args.seed
    )
    
    # 인덱스 정보 저장
    with open(f"{args.model_path}/dropped_indices.pickle", "wb") as fb:
        pickle.dump(dropped_indices, fb)
    with open(f"{args.model_path}/train_idx.pickle", "wb") as fb:
        pickle.dump(train_idx, fb)
    with open(f"{args.model_path}/test_idx.pickle", "wb") as fb:
        pickle.dump(test_idx, fb)
    
    # train_rate=0.01일 때의 인덱스 정보 저장
    if args.train_rate == 0.01:
        print(f"\nSaving indices for train_rate=0.01")
        print(f"Train indices shape: {train_idx.shape}")
        print(f"Test indices shape: {test_idx.shape}")
        with open(f"{args.model_path}/train_idx_0.01.pickle", "wb") as fb:
            pickle.dump(train_idx, fb)
        with open(f"{args.model_path}/test_idx_0.01.pickle", "wb") as fb:
            pickle.dump(test_idx, fb)
